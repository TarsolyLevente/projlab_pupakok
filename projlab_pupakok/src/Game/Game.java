...
public class Game {
    ...
    Timer timer = new Timer(1, e -> szamlalo++);
     ...
     * for (inventorythrowcnt = 0; inventorythrowcnt < hallgato.getTaska().size();
     * inventorythrowcnt++) {
     * System.out.println(
     * inventorythrowcnt + 1 + ". "
     ...
     * tpcnt++;
     * System.out.println(tpcnt + ". Teleportálás " + t.toString(Funkcio.aktiv) +
     * "ral");
     * dict.put(tpcnt, i);
     * }
     * }
     ...
     * szomszedcnt++;
     * System.out.println(szomszedcnt + ". " + szoba.getid() +
     * ". számú szobába lépés");
     * }
     * 
     * int data = 0;
     ...
     * hallgato.felvesz(hallgato.getSzoba().getTargyak().get(data - inventoryusecnt
     * - 1));
     * else if (data <= tpcnt)
     * hallgato.teleport((Tranzisztor)
     * hallgato.getSzoba().getTargyak().get(dict.get(data)));
     * else {
